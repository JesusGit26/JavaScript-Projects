// This function will get fired once the DOMis loaded.
...
function startGame() {
    ...
    setTimeout(function() {hideGameMsg();}, 5000);

    // assign proper state of the control buttons
    var btn = document.getElementById('btnStart');
    btnDisabled(btn); //disable the start button
    var btn = document.getElementById('btnStop');
...
// proposed avatar assignments and prevents them from being the smae
...
// this function will return the currently assigned avatar from each player
function getAvatars() {
...
function avatarPlaced() {
    ...
    var showPlayer = document.getElementById("showPlayer");
    if (parseText == "That's a three in a row, Player 1 wins!" || parseText == "That's three in a row, Player 2 wins!") {
        showPlayer.innerHTML = "Game Stopped";
        showPlayer.style.color = "red";
    }
...
// as squares are selectedthey checkin with this function to see if that particular
// square has already been assigned and if it has not, recored new square with assigned avatar
...
function recordMove(currentMove) {
    ...
    target.innerText = previousMoves+currentMove;
}
...
function check4Tie() {
    ...
    if(boardState.length >= 9 && check != "That's three in a row, Player 1 wins!" && check != "That's three is a row, Player 2 wins!") {
...
//call this function to produce the following winning the progress for the game
...
function glowBoard(pos) {
    ...
    for (var i=0; i<squares.length; i++) {
        if (i == index0) {
            ...
            setTimeout(function() {bg1.style.backgroundColor = 'rgb(197, 179, 66)';}, 300);
            ...
            setTimeout(function() {bg3.style.backgroundColor = 'rgb(66, 244, 66)';}, 900);
...
// flash for a few seconds for a win nimation
function blink() {
